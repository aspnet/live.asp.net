...
namespace live.asp.net.Formatters
{
    public class iCalendarOutputFormatter : OutputFormatter
    {
        ...
        public override Task WriteResponseBodyAsync(OutputFormatterContext context)
        {
            ...
            Debug.Assert(liveShowDetails != null, "Object to be formatted should be of type LiveShowDetails");
        ...
        private static int _footerOffset =
            ...
            Encoding.UTF8.GetByteCount(_dateTimeFormat + Environment.NewLine + "DTEND:" + _dateTimeFormat);
        ...
        private static Task WriteLiveShowDetailsToResponseBody(LiveShowDetails liveShowDetails, Stream body)
        {
            ...
            var payload = Encoding.UTF8.GetBytes(
                liveShowDetails.NextShowDateUtc.Value.ToString(_dateTimeFormat) + Environment.NewLine + "DTEND:" +
                (liveShowDetails.NextShowDateUtc.Value.AddMinutes(30)).ToString(_dateTimeFormat));

            var responseBuffer = new byte[_payloadLength];

            Buffer.BlockCopy(_iCalHeaderBytes, 0, responseBuffer, 0, _iCalHeaderBytes.Length);
            Buffer.BlockCopy(payload, 0, responseBuffer, _iCalHeaderBytes.Length, payload.Length);
